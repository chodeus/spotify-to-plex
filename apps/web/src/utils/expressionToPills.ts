    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token === 'AND' || token === 'OR') {
            ...
            if (condition) {
                ...
                const fieldOnlyPattern = /^(artist|title|album|artistWithTitle|artistInTitle|duration|version)$/;
...
function parseCondition(conditionText: string): ParsedCondition | null {
    ...
    const conditionPattern = /^(artist|title|album|artistWithTitle|artistInTitle|duration|version):(match|contains|similarity(?:>=\d*\.?\d+)?)$/;
